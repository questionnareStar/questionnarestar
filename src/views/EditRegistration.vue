...
<div class="container-fluid p-20">
                ...
                <div class="header">
                    <div class="mouseon">
                        <div class="center-block" @click="titleEdit=!titleEdit">
                            <el-input v-if="titleEdit" v-focus type="textarea" autosize @blur="titleEdit=false" v-model="data.head" />
                    ...
                    <div class="mouseon">
                        <div class="center-block" @click="desEdit=!desEdit">
                            <el-input v-if="desEdit" v-focus type="textarea" :autosize="{ minRows: 2, maxRows: 4}" @blur="desEdit=false" v-model="data.introduction" />
                            ...
                            <div v-for="(item, index) in data.questions" :key="index" class="cursoron">
                                ...
                                <div v-if="item.type === 1">
                                    <el-row class="mb-8">
                                        ...
                                        <span class="question red" v-if="item.required">*</span>
                                    </el-row>
                                    <el-row class="mb-8">
                                        <span class="des">{{item.desc}}</span>
                                    </el-row>
                                    <el-row class="mb-8">
                                        <el-input type="textarea" autosize disabled placeholder="此处作答" />
                                    </el-row>
                                </div>
                                <!-- 评分题 -->
                                ...
                                <div class="cursoron" v-if="item.type === 3">
                                    ...
                                    <el-row class="mb-8">
                                        <el-checkbox class="choice-block" v-for="(choice, idx) in item.choices" :key="idx" :label="choice.value">{{choice.value}}</el-checkbox>
                                    </el-row>
                                </div>
                                <!-- 单选题 -->
                                <div class="cursoron" v-if="item.type === 4">
                                    ...
                                    <el-row class="mb-8">
                                        <el-radio class="choice-block" v-for="(choice, idx) in item.choices" :key="idx" :label="choice.value">{{choice.value}}</el-radio>
                                    </el-row>
                                </div>
                                <el-row>
    ...
    <!-- 填空题 -->
    <el-dialog title="编辑填空题" :visible.sync="blank.visible" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
        <el-form :model="blank">
    ...
    <!-- 评分题 -->
    <el-dialog title="编辑评分题" :visible.sync="mark.visible" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
        <el-form :model="mark">
            <el-form-item label="题目">
    ...
    <!-- 多选题 -->
    <el-dialog title="编辑多选题" :visible.sync="multi.visible" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
        <el-form :model="multi">
            <el-form-item label="题目">
    ...
    <!-- 单选题 -->
    <el-dialog title="编辑单选题" :visible.sync="single.visible" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
            ...
            for (let i = 0; i < this.data.questions.length; i++) {
                ...
                switch (item.type) {
                    case 1:
                        ...
                        if (item.original) {
                            await survey.updateBlank(formData).then((response) => {
                                this.questionResponseHandle(response, i, 1)
                            });
                    ...
                    case 2:
                        ...
                        if (item.original) {
                            await survey.updateMark(formData).then((response) => {
                                this.questionResponseHandle(response, i, 2)
                            });
                        } else {
                    ...
                    case 3:
                        ...
                        if (item.original) {
                            await survey.updateMulti(formData).then((response) => {
                                this.questionResponseHandle(response, i, 3)
                            });
                        } else {
                    ...
                    case 4:
                        ...
                        if (item.original) {
                            await survey.updateSingle(formData).then((response) => {
                                this.questionResponseHandle(response, i, 4)
                            });
                        } else {
        ...
        async submit() {
            ...
            if (this.wholeEdit) {
                await survey
                    ...
                    .then((response) => {
                        this.questionnareResponseHandle(response)
                    })
                    .catch((error) => {
