<template>
    <div class="container-fluid p-20">
                        ...
                        <draggable v-model="data.questions"  @start="onStart" @end="onEnd">
                            <transition-group>
                                <div v-for="(item, index) in data.questions" :key="index" class="cursoron">
                                    ...
                                    <!-- 定位题 -->
                                    <div class="cursoron" v-if="item.type === 7">
                                        <el-row class="mb-8">
                                            <span class="question" v-if="data.serial">{{index + 1}}</span>
                                            <span class="question">{{item.question}}</span>
                                            <span class="question red"
            ...
            <div class="col-md-4">
                <div class="ml-10 panel">
                    ...
                    <div class="shadow p-5 mb-20">
                        ...
                        <el-row class="ml-40 mb-8 p-5">
                            <el-button type="primary" plain @click="locate.visible = true"><em class="el-icon-plus mr-5"/>定位题</el-button>
                        </el-row>
                    </div>
                </div>
            </div>
        ...
        <el-dialog
            title="编辑定位题"
            :visible="locate.visible"
            :close-on-click-modal="false"
            :close-on-press-escape="false"
            :show-close="false">
            <el-form :model="locate">
                <el-form-item label="题目">
                    <el-input v-model="locate.question" autocomplete="off"/>
                </el-form-item>
                <el-form-item label="描述">
                    <el-input v-model="locate.desc" placeholder="此处输入题目描述" autocomplete="off"/>
                </el-form-item>
                <el-form-item>
                    <el-switch v-model="locate.required" active-text="必答"/>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click.native="addLocate()">确 定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
/**
  新增定位题，type=7
  后端将其视为填空题
  在调用接口是在question字段的字符串前添加一段'【locating】'加以区分
 */
...
export default {
            ...
            for (const item of data.itemList) {
              ...
              switch (item.itemType) {
                case 1:
                  ...
                  if (item.question.indexOf(locStr) !== -1) {
                    q.type = 7
                    q.question = item.question.substring(10)
                  }
      ...
      data: {
        ...
        questions: [
            ...
            },
            {
                ...
                question: '体温范围',
                required: true,
                choices: [{ value: '正常(37.2℃及以下)' }, { value: '37.3℃~38℃' }, { value: '38.1℃~38.5℃'}, { value: '38.6℃~39℃'}, { value: '39.1℃~40℃' }, { value: '40.1℃以上' }],
                original: false
            },
            {
                type: 4,
  ...
  methods: {
    ...
    addLocate() {
      let q = {
        type: 7,
        desc: this.locate.desc,
        question: this.locate.question,
        required: this.locate.required,
        original: false
      }
      if (this.editIndex == -1) {
        this.data.questions.push(q);
