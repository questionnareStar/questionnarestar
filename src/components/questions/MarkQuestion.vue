<template>
    <div style="width=85% mb-8 height=100px">
        <ValidationProvider :rules="`${required ? 'mark' : ''}`" v-slot="{ errors }">
            ...
            <el-row class="question-choices">
                <el-rate v-model="answers[0]" :maxScore="maxScore" @change="onchange()"/>
            </el-row>
...
<script>
export default {
    props: ['id', 'serial', 'sequence', 'question', 'required', 'description', 'maxScore'],
    data() {
        return {
            answers: []
        }
